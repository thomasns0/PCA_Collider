...
#adjusted here to sample case/control trait from rbinom

#normally distributed probabilities, probit
scaledZ_pc1_Unobserved<-(as.numeric(scale((pc1_Unobserved))))*UnObsToPheno
scaledZ_Environment<-(as.numeric(scale((Environment))))*EnvToPheno
scaledZ_PRS<-(as.numeric(scale((PRS))))*PRStoPheno
normalProb<-pnorm(q=scaledZ_pc1_Unobserved+scaledZ_Environment+scaledZ_PRS, mean=0,sd=1)

#logistic distributed probabilities, logit
scaledLogis_pc1_Unobserved<-qlogis(pnorm(scaledZ_pc1_Unobserved))
scaledLogis_Environment<-qlogis(pnorm(scaledZ_Environment))
scaledLogis_PRS<-qlogis(pnorm(scaledZ_PRS))
LogisProb<-plogis(q=scaledLogis_pc1_Unobserved+scaledLogis_Environment+scaledLogis_PRS)

Pheno<-rbinom(n=1000, size=1, prob= LogisProb)
...
#cor(df)
Mod_PhenPRS<-glm(Pheno ~ PRS, data=df, family=binomial(link="logit"))
Mod_PhenoPrsEnv<-glm(Pheno ~ PRS + Environment, data=df,family=binomial(link="logit"))
Mod_PhenoPrsEnvUnobsPC<-glm(Pheno ~ PRS + Environment + pc1_Unobserved, data=df,family=binomial(link="logit"))
Mod_PhenoPrsEnvObsPC<-glm(Pheno ~ PRS + Environment + pc1_Observed, data=df,family=binomial(link="logit"))
PhenPRS<-summary(Mod_PhenPRS)$coef["PRS",c("Estimate", "Pr(>|z|)")]
PhenoPrsEnv<-summary(Mod_PhenoPrsEnv)$coef["PRS",c("Estimate", "Pr(>|z|)")]
PhenPrsEnvUnobsPC<-summary(Mod_PhenoPrsEnvUnobsPC)$coef["PRS",c("Estimate", "Pr(>|z|)")]
PhenPrsEnvObsPC<-summary(Mod_PhenoPrsEnvObsPC)$coef["PRS",c("Estimate", "Pr(>|z|)")]
...
outputfile<-ALLoutB'
  ...
  Rscript2<-
    paste0(
      "
  write.table(outputfile,", paste0('"',"/vcu_gpfs2/home/thomasns/ColliderBiasPhRM/Logistic_ALLoutB", "_", i,".csv", '"'), ",sep=',', col.names = FALSE, row.names = FALSE, quote = FALSE)
  "
    )
  Rscript<-paste0(Rscript1, Rscript2)
  
  write.table(Rscript, paste0('/vcu_gpfs2/home/thomasns/ColliderBiasPhRM/Logistic_ALLoutB',"_", i, '.R'), sep=",", col.names = FALSE, row.names = FALSE, quote = FALSE)
  scriptNames<-rbind(scriptNames,paste0(directory, '/Logistic_ALLoutB',"_", i, '.R'))
  OutputNames<-rbind(OutputNames, paste0(directory, '/Logistic_ALLoutB',"_", i, '.csv'))
...
R CMD BATCH ${R_SCRIPT} $1 > Rout 2> Rerr'
  ...
  script <- paste0(directory, '/Logistic_ALLoutB',"_", i, '.sh')
