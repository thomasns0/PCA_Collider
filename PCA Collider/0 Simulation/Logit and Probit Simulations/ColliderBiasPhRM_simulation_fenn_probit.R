...
Pheno<-rbinom(n=1000, size=1, prob= normalProb)
...
#cor(df)
Mod_PhenPRS<-glm(Pheno ~ PRS, data=df, family=binomial(link="probit"))
Mod_PhenoPrsEnv<-glm(Pheno ~ PRS + Environment, data=df,family=binomial(link="probit"))
Mod_PhenoPrsEnvUnobsPC<-glm(Pheno ~ PRS + Environment + pc1_Unobserved, data=df,family=binomial(link="probit"))
Mod_PhenoPrsEnvObsPC<-glm(Pheno ~ PRS + Environment + pc1_Observed, data=df,family=binomial(link="probit"))
...
outputfile<-ALLoutB'
  ...
  Rscript2<-
    paste0(
      "
  write.table(outputfile,", paste0('"',"/vcu_gpfs2/home/thomasns/ColliderBiasPhRM/Probit_ALLoutB", "_", i,".csv", '"'), ",sep=',', col.names = FALSE, row.names = FALSE, quote = FALSE)
  "
    )
  Rscript<-paste0(Rscript1, Rscript2)
  
  write.table(Rscript, paste0('/vcu_gpfs2/home/thomasns/ColliderBiasPhRM/Probit_ALLoutB',"_", i, '.R'), sep=",", col.names = FALSE, row.names = FALSE, quote = FALSE)
  scriptNames<-rbind(scriptNames,paste0(directory, '/Probit_ALLoutB',"_", i, '.R'))
  OutputNames<-rbind(OutputNames, paste0(directory, '/Probit_ALLoutB',"_", i, '.csv'))
...
R CMD BATCH ${R_SCRIPT} $1 > Rout 2> Rerr'
  ...
  script <- paste0(directory, '/Probit_ALLoutB',"_", i, '.sh')
